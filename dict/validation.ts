...
const ApiAppointmentDtKeys: string[] = ['id', 'patientid', 'doctorid', 'doctorName', 'doctorImage', 'createdAt', 'symptoms', 'slotid', 'slotTime', 'appointmentDate', 'appointmentType', 'appointmentStatus']

function modelApiAppointmentDtFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.AppointmentDt {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiAppointmentDtKeys.forEach(k => knownKeys[k] = true)

	const result: Api.AppointmentDt = {
		'id': allowUndefined(stringFromJson)(`${name}.id`, value['id']),
		'patientid': stringFromJson(`${name}.patientid`, value['patientid']),
		'doctorid': stringFromJson(`${name}.doctorid`, value['doctorid']),
		'doctorName': allowUndefined(stringFromJson)(`${name}.doctorName`, value['doctorName']),
		'doctorImage': allowUndefined(stringFromJson)(`${name}.doctorImage`, value['doctorImage']),
		'createdAt': allowUndefined(stringFromJson)(`${name}.createdAt`, value['createdAt']),
		'symptoms': stringFromJson(`${name}.symptoms`, value['symptoms']),
		'slotid': stringFromJson(`${name}.slotid`, value['slotid']),
		'slotTime': stringFromJson(`${name}.slotTime`, value['slotTime']),
		'appointmentDate': stringFromJson(`${name}.appointmentDate`, value['appointmentDate']),
		'appointmentType': numberFromJson(`${name}.appointmentType`, value['appointmentType']),
...
function modelApiAppointmentDtToJsonContent(name: string, value: Api.AppointmentDt, knownKeys: Record<string, boolean> = {}): ToJson<Api.AppointmentDt> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.AppointmentDt> = {
		'id': allowUndefined(stringToJson)(`${name}.id`, value['id']),
		'patientid': stringToJson(`${name}.patientid`, value['patientid']),
		'doctorid': stringToJson(`${name}.doctorid`, value['doctorid']),
		'doctorName': allowUndefined(stringToJson)(`${name}.doctorName`, value['doctorName']),
		'doctorImage': allowUndefined(stringToJson)(`${name}.doctorImage`, value['doctorImage']),
		'createdAt': allowUndefined(stringToJson)(`${name}.createdAt`, value['createdAt']),
		'symptoms': stringToJson(`${name}.symptoms`, value['symptoms']),
		'slotid': stringToJson(`${name}.slotid`, value['slotid']),
		'slotTime': stringToJson(`${name}.slotTime`, value['slotTime']),
		'appointmentDate': stringToJson(`${name}.appointmentDate`, value['appointmentDate']),
		'appointmentType': numberToJson(`${name}.appointmentType`, value['appointmentType']),
...
export function modelApiAppointmentDtFromJson(name: string, value: any): Api.AppointmentDt {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.AppointmentDt = modelApiAppointmentDtFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			...
			console.warn(`Unexpected key in Api.AppointmentDt: ${key}`)
		}
	}
...
export function modelApiAppointmentDtToJson(name: string, value: Api.AppointmentDt): ToJson<Api.AppointmentDt> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.AppointmentDt> = modelApiAppointmentDtToJsonContent(name, value, knownKeys)
...
const ApiAppointmentPagedResultDtoKeys: string[] = ['totalCount', 'items']

function modelApiAppointmentPagedResultDtoFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.AppointmentPagedResultDto {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiAppointmentPagedResultDtoKeys.forEach(k => knownKeys[k] = true)

	const result: Api.AppointmentPagedResultDto = {
		'totalCount': allowUndefined(integerFromJson)(`${name}.totalCount`, value['totalCount']),
		'items': allowUndefined(arrayFromJson(modelApiAppointmentDtFromJson))(`${name}.items`, value['items']),
	}

	return result
}

function modelApiAppointmentPagedResultDtoToJsonContent(name: string, value: Api.AppointmentPagedResultDto, knownKeys: Record<string, boolean> = {}): ToJson<Api.AppointmentPagedResultDto> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.AppointmentPagedResultDto> = {
		'totalCount': allowUndefined(integerToJson)(`${name}.totalCount`, value['totalCount']),
		'items': allowUndefined(arrayToJson(modelApiAppointmentDtToJson))(`${name}.items`, value['items']),
	}

	return result
}

export function modelApiAppointmentPagedResultDtoFromJson(name: string, value: any): Api.AppointmentPagedResultDto {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.AppointmentPagedResultDto = modelApiAppointmentPagedResultDtoFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			// throw `Unexpected key: ${key}`
			console.warn(`Unexpected key in Api.AppointmentPagedResultDto: ${key}`)
		}
	}
...
export function modelApiAppointmentPagedResultDtoToJson(name: string, value: Api.AppointmentPagedResultDto): ToJson<Api.AppointmentPagedResultDto> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.AppointmentPagedResultDto> = modelApiAppointmentPagedResultDtoToJsonContent(name, value, knownKeys)
